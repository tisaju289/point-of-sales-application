...
<script>
    ...
    async function Update() {
        ...
        if (title.length === 0 || vacancy.length === 0 || salary.length === 0) {
            errorToast("Title, Vacancy, and Salary are required!");
        } else {
            document.getElementById('update-modal-close').click();
            showLoader();
            let res = await axios.post("/update-job", {title: title, vacancy: vacancy, salary: salary, id: updateID});
            hideLoader();
            if (res.status === 200 && res.data === 1) {
                successToast('Job details updated successfully');
                document.getElementById("update-form").reset();
                await getList(); // Assuming getList() is defined somewhere else
            } else {
                errorToast("Failed to update job details");
            }
        }
    }
</script>
